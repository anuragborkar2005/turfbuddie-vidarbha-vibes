    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 404) {
          throw new Error(`Geolocation API endpoint not found at /api/geocode. Status: ${response.status}`);
        } else {
          throw new Error(`Geolocation API call failed with status: ${response.status}`);
        }
      }
  ...
  const filtered = useMemo(() => {
   ...
   function handleGeolocationError(error: GeolocationPositionError) {
    switch (error.code) {
      ...
      default:
        console.error(`An unknown geolocation error occurred: ${error.message}`);
    }
  }
